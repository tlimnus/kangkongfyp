...
LR = 1e-3            # Higher LR is fine for a linear probe
...
# With 53 images, this is the safest choice.
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_acc:
        ...
        torch.save({
            ...
            'best_acc': best_acc,
        }, "kangkongjr_dinov2_best.pth")
        print(f"  → Saved best model (Val Acc: {best_acc:.1f}%)")
    else:
        patience_counter += 1
...
print(f"Model saved to: kangkongjr_dinov2_best.pth")
print(f"{'='*50}")
...
# Load model
checkpoint = torch.load("kangkong_dinov2_best.pth")
backbone = torch.hub.load("facebookresearch/dinov2", checkpoint['backbone_name'])
backbone.eval().to(device)

classifier = nn.Linear(checkpoint['embed_dim'], checkpoint['num_classes']).to(device)
...
with torch.no_grad():
    ...
    pred_class = checkpoint['classes'][probs.argmax().item()]
